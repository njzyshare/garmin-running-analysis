...
> 用于训练分析时的指标解读依据。以下所有指标在报告中使用中文表述，英文缩写仅作补充说明。

---

## 一、身体电量 / Body Battery（0-100）

Garmin 基于心率变异性(HRV)、压力、睡眠和活动的专有恢复指标。
...
- **趋势比绝对值更重要** — 比较周/月平均值
- **心率变异性下降趋势**（连续 3+ 天低于基线 10%）：累积压力、恢复差、过度训练前兆
- **急性下降**：睡眠差、饮酒、疾病、高强度训练、情绪压力

---

## 四、静息心率 / Resting Heart Rate（bpm）
...
- **连续低心率变异性(HRV)**（<15ms）→ 建议就医
- **睡眠呼吸率 >20 次/分** → 可能的呼吸问题
- **静息心率连续 3+ 天升高 5+ bpm** → 注意疾病/过度训练
- **身体电量(Body Battery)连续 5+ 天峰值 <75** → 慢性恢复不足
