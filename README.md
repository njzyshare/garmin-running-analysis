...
> 以下按版本号从新到旧列出 Skill 的历次重大功能演进。

### ⌚ v2.3.0 — 间歇训练深度分析 (2026-07-09)
_恢复段心率评估 · 等强配速短圈失真修复_

| 变化 | 说明 |
|:---|:---|
| ✨ 间歇恢复段心率评估 | 每段间歇后的恢复圈取最后5秒最低心率，量化"有没有把心率降下去" |
| 🔧 等强配速间歇场景屏蔽 | 短计圈（200m/400m）上坡度调整配速失真，间歇训练+平坦路线不再展示 |
| 🔧 分段拆解模板通用化 | 热身→核心→恢复→冷身的四段模板，不限于间歇跑 |

---

### 🌤 v2.2.1 — 坡度调整配速 & 睡眠规则 (2026-06-17)
_优先 Garmin 内置字段 · 睡眠日期交叉分析_

| 变化 | 说明 |
|:---|:---|
| ✨ 等强配速优先内置 | 优先读取 Garmin `avgGradeAdjustedSpeed`，爬升>10m才展示 |
| 🐛 睡眠关联规则修正 | D日睡眠差不再直接归因训练，先排查D日清晨是否早起训练 |
| 🔧 术语规范化 | HRV 保留英文，其余指标全部中文化；移除 COROS 残留引用 |

---

### 🀄 v2.2.0 — 报告本地化 (2026-05-20)
_指标全中文 · 面向国内跑者_

| 变化 | 说明 |
|:---|:---|
| ✨ 全中文指标 | 报告术语（除 HRV 外）全部使用中文，适配国内跑者阅读习惯 |

---

### 🏗 v2.0.x — 基础设施完善 (2026-05-19)
_等强配速 · 天气量化 · PB提取 · 中国区登录_

| 变化 | 说明 |
|:---|:---|
| ✨ 等强配速 | 三级折算方案（逐公里坡度→心率修正→Naismith），准确评估真实负荷 |
| ✨ 天气量化 | 不适指数(DI)分区间统计，量化高温高湿对配速的影响 |
| ✨ PB 提取 | 从活动记录提取 PB，作为 Garmin API fallback |
| ✨ 训练评分 | 跑量/长距离/配速/频率四维评分系统 |
| ✨ 中国区支持 | Garmin.cn monkey-patch 登录，国内用户零门槛 |
| 🔧 项目重构 | SKILL.md 结构化拆分，新增 references/ 参考文件体系 |

---

### 🔄 v2.0.0 — 训练体系重构 (2026-04-12)
_科学训练法 · 自动计圈识别_

| 变化 | 说明 |
|:---|:---|
| ✨ 四大训练体系 | 丹尼尔斯 VDOT、MAF 180、汉森累积疲劳、亚索 800 |
| ✨ 计圈启发式分析 | 自动识别间歇跑/节奏跑/异常模式，不再靠硬编码规则 |
| 🔧 Skill 瘦身 | 技术细节剥离到 references/，SKILL.md 保持精炼 |

---

### 🚀 v1.0.0 — 初始发布 (2026-03-26)
_数据获取 → 报告生成 → 天气关联 完整链路_

| 变化 | 说明 |
|:---|:---|
| ✨ 数据获取 | Garmin Connect 活动/睡眠/HRV/身体电量全量读取 |
| ✨ 训练报告 | 配速、心率、爬升等核心指标 HTML 可视化 |
| ✨ 天气关联 | 自动获取训练区域天气，温度/湿度/风力关联配速分析 |
| ✨ 文件下载 | FIT/GPX/TCX 格式的活动记录下载与解析 |
