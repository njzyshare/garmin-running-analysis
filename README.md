...
支持中国区（garmin.cn）和国际区（garmin.com）账号，自动获取训练记录、睡眠、HRV、Body Battery 等核心指标。
...
创建 `~/.clawdbot/garmin-config/config.json`：
...
# 综合摘要
python scripts/garmin_data.py summary --days 14
```
...
# 自定义分析周期
python scripts/garmin_report.py --days 14

# 指定输出路径
python scripts/garmin_report.py --output ~/Desktop/report.html
```
...
# 血氧
python scripts/garmin_data_extended.py spo2

# 体能年龄
python scripts/garmin_data_extended.py fitness_age
```
...
三、当月整体评估          # 月度天气变化、DI 分布、恢复趋势
四、优化建议              # 基于四维度评分的训练建议
五、课表建议              # 基于当前状态的全马备赛建议
六、后续分析选项          # 分段/周/月分析引导
```
...
.
├── scripts/
│   ├── garmin_auth.py           # 登录认证（支持中国区 monkey-patch）
│   ├── garmin_data.py          # 健康数据获取（睡眠/HRV/BB/心率/活动/压力）
│   ├── garmin_chart.py         # HTML 图表生成（Chart.js）
│   ├── garmin_report.py        # 综合训练分析报告（COROS 风格）
│   ├── garmin_query.py         # 时间点查询（心率/压力/BB/步数）
│   ├── garmin_data_extended.py # 扩展指标（训练准备度/身体成分/血氧等）
│   └── garmin_activity_files.py # 活动文件下载/解析（FIT/GPX/TCX）
├── docs/
│   └── 技术参考.md              # 详细技术文档
├── .gitignore
└── README.md
...
- **Token 存储**：登录后 Token 保存在 `~/.clawdbot/garmin/` 目录
- **频率限制**：Garmin API 有请求限制，避免频繁轮询
- **中国区兼容**：内置 garmin.cn 认证 monkey-patch，自动处理中国区登录流程
