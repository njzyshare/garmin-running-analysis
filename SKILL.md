...
description: 查询 Garmin 健康数据并生成交互式 HTML 图表。支持 20+ 指标（睡眠阶段、身体电量/Body Battery、心率变异性/HRV、VO₂ max、训练准备度、身体成分、血氧），下载 FIT/GPX 路线文件，查询任意时间点的海拔/配速，生成训练分析报告，并提供天气/温湿度量化分析、不适指数/DI 评估。从"这周训练怎么样？"到"深度分析我的恢复 vs 训练负荷"。
version: 2.2.0
...
| 脚本 | 用途 | 常用参数 |
|------|------|---------|
| `garmin_data.py` | 活动/睡眠/心率变异性/身体电量/心率 | `activities --days 30`、`sleep --days 14`、`hrv --days 30`、`body_battery --days 30`、`summary --days 7`、`profile` |
...
- **恢复与健康趋势**：心率变异性(HRV)、身体电量(Body Battery)、静息心率变化
...
| 首次分析 | 并行获取 profile + 7天摘要 + 30天活动 + 心率变异性(HRV) + 训练准备度 + VO₂max + 身体电量(Body Battery) |
...
| `metrics_reference.md` | 身体电量(Body Battery) / 睡眠评分 / 心率变异性(HRV) / 静息心率的解读参考 |
...
v2.2.0 — 术语中文化：全部指标使用中文表述（身体电量、心率变异性、不适指数、静息心率），英文缩写仅作补充说明。依赖：garminconnect、fitparse、gpxpy。协议：MIT。
