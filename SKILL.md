...
| 脚本 | 用途 | 常用参数 |
|------|------|---------|
| `garmin_data.py` | 活动/睡眠/HRV/BB/心率 | `activities --days 30`、`sleep --days 14`、`hrv --days 30`、`body_battery --days 30`、`summary --days 7`、`profile` |
...
- **恢复与健康趋势**：HRV、Body Battery、静息心率变化
- **天气量化分析**：DI 分区间统计、高温高湿 vs 舒适日配速偏差对比
...
| 首次分析 | 并行获取 profile + 7天摘要 + 30天活动 + HRV + 训练准备度 + VO₂max + Body Battery |
...
| `metrics_reference.md` | Body Battery / 睡眠评分 / HRV / 静息心率的解读参考 |
...
v2.1.0 — 依赖：garminconnect、fitparse、gpxpy。协议：MIT。
